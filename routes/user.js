...
router.patch('/', async (req, res) => {
    ...
    let isMatch
    try {
        isMatch = await User.comparePassword(userId, currentPassword)
    ...
    let isChanged
    try {
        isChanged = await User.updatePassword(userId, newPassword)
...
router.delete('/:id/', async (req, res) => {
    ...
    try{
       ...
       return res.status(500).json({err: "در این لحظه امکان حذف اکانت وجود ندارد"})
    }
