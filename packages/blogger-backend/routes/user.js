...
router.post("/users", notLoggedIn, async (req, res) => {
  ...
  if (!isDataValid) {
    return res.status(400).send(".مقادیر ورودی را چک کنید");
  }

  try {
    await User.create(req.body);
    res.status(201).send(".اکانت شما با موفقیت ساخته شد");
  } catch (err) {
    res.status(500).send(".مشکلی در ساخت اکانت شما وجود دارد");
  }
});
...
router.put("/users/:id/", isLoggedIn, (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ err: "تغییرات نا موفق بود" });
  }
});
...
router.patch("/users", isLoggedIn, async (req, res) => {
  ...
  try {
    ...
    if (err.errors.password.kind === "minlength")
      return res.status(400).send("رمز جدید باید بزرگتر از 5 حرف باشد");
...
router.delete("/users/:id", isLoggedIn, async (req, res) => {
  ...
  try {
    ...
    return res.status(500).send("در این لحظه امکان حذف اکانت وجود ندارد");
  }
});
...
router.post("/users/avatar", isLoggedIn, async (req, res) => {
  ...
  uploadAvatar(req, res, function (err) {
    ...
    try {
      let isAvatarChanged = User.changeAvatar(
...
router.get("/users/:id/resetpassword", async (req, res) => {
  try {
    ...
    res.status(500).send("مشکلی در بازیابی رمز عبور وجود دارد.");
  }
});
